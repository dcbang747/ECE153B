...
void RTC_Init(void) {
	...
	// Configure the Time 
	RTC_Set_Time(RTC_TR_PM, 0x03, 0x20, 0x00); /* [TODO] These values are stubs - fill in current time */
...
{
	uint32_t dr  = 0;
	dr |= (((Year  >> 4) & 0x0F) << RTC_POSITION_DR_YT)  |
	(( Year        & 0x0F) << RTC_POSITION_DR_YU)  |
	( WeekDay              << RTC_POSITION_DR_WDU) |
	(((Month >> 4) & 0x01) << RTC_POSITION_DR_MT)  |
	(( Month       & 0x0F) << RTC_POSITION_DR_MU)  |
	(((Day   >> 4) & 0x03) << RTC_POSITION_DR_DT)  |
	(( Day         & 0x0F) << RTC_POSITION_DR_DU);
	RTC->DR = dr;
}

void RTC_Set_Time(uint32_t Format12_24, uint32_t Hour, uint32_t Minute, uint32_t Second)
{
	uint32_t tr  = Format12_24;                              /* keeps PM bit if set        */
	tr |= (((Hour   >> 4) & 0x03) << RTC_POSITION_TR_HT) |
	(( Hour         & 0x0F) << RTC_POSITION_TR_HU) |
	(((Minute >> 4) & 0x07) << RTC_POSITION_TR_MT) |
	(( Minute       & 0x0F) << RTC_POSITION_TR_MU) |
	(((Second >> 4) & 0x07) << RTC_POSITION_TR_ST) |
	(( Second       & 0x0F) << RTC_POSITION_TR_SU);
	RTC->TR = tr;
}
...
{
    RTC->WPR = 0xCA;          /* key sequence: 0xCA, 0x53 */
    RTC->WPR = 0x53;
}

void RTC_Enable_Write_Protection(void)
{
    RTC->WPR = 0xFF;          /* any other value re‑locks the registers */
}
...
{
    uint32_t tr = RTC->TR;
    return (((tr & RTC_TR_HT) >> RTC_POSITION_TR_HT) << 4) |
            ((tr & RTC_TR_HU) >> RTC_POSITION_TR_HU);
}

uint32_t RTC_TIME_GetMinute(void)
{
    uint32_t tr = RTC->TR;
    return (((tr & RTC_TR_MNT) >> RTC_POSITION_TR_MT) << 4) |
            ((tr & RTC_TR_MNU) >> RTC_POSITION_TR_MU);
}

uint32_t RTC_TIME_GetSecond(void)
{
    uint32_t tr = RTC->TR;
    return (((tr & RTC_TR_ST) >> RTC_POSITION_TR_ST) << 4) |
            ((tr & RTC_TR_SU) >> RTC_POSITION_TR_SU);
}

uint32_t RTC_DATE_GetMonth(void)
{
    uint32_t dr = RTC->DR;
    return (((dr & RTC_DR_MT) >> RTC_POSITION_DR_MT) << 4) |
            ((dr & RTC_DR_MU) >> RTC_POSITION_DR_MU);
}

uint32_t RTC_DATE_GetDay(void)
{
    uint32_t dr = RTC->DR;
    return (((dr & RTC_DR_DT) >> RTC_POSITION_DR_DT) << 4) |
            ((dr & RTC_DR_DU) >> RTC_POSITION_DR_DU);
}

uint32_t RTC_DATE_GetYear(void)
{
    uint32_t dr = RTC->DR;
    return (((dr & RTC_DR_YT) >> RTC_POSITION_DR_YT) << 4) |
            ((dr & RTC_DR_YU) >> RTC_POSITION_DR_YU);
}
